...
def gather_by_label(out, labels):
    ...
    for label_index in unique_labels[1:]:
        mask = torch.tensor(labels == label_index).unsqueeze(1).float()
...
def gather_by_label_matrix(out, labels):
    ...
    out = torch.div(out, torch.clamp(norm, min=1.0))
    return out


def gather_by_one_hot_labels(out, labels):
    b, c, h, w = out.shape

    out = out.view(b, c, h * w)
    labels = labels.view(b, h * w, -1)
    features = torch.bmm(out, labels)

    sum_mask = torch.sum(labels, dim=1, keepdim=True)
    sum_mask = torch.clamp_min(sum_mask, min=1.0)
    features = features / sum_mask
