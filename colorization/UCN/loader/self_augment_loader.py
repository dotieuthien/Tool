...
from loader.mask_utils import load_comps_from_mask
...
class PairAnimeDataset(data.Dataset):
    def __init__(self, root_dir, size, config, kpts=True):
        ...
        self.all_sketch_set_names = ["sketch"]
        ...
        for sub_dir in dirs:
            ...
            for set_name in all_set_names:
                paths = []
                for sub_type in ["png", "jpg", "tga", "TGA"]:
    ...
    def get_component_mask(self, color_image, sketch, path, method):
        ...
        # TODO: change to store mask only
        dirpath = "/".join(path.split(os.sep)[:-2])
        save_path = os.path.join(dirpath, 'annotations', '%s_%s.png' %('sketch', name))
        # save_path = os.path.join(os.path.dirname(path), "%s_%s.PNG" % (name, method))
        if os.path.exists(save_path):
            try:
                mask = imageio.imread(save_path)
                components = load_comps_from_mask(mask)
                RE_EXTRACT = False
            except:
                RE_EXTRACT = True
        else:
        ...
        if RE_EXTRACT: # not os.path.exists(save_path):
            # Folder annotations
            if not os.path.exists(os.path.join(dirpath, 'annotations')):
                os.mkdir(os.path.join(dirpath, 'annotations'))
    ...
    def __getitem__(self, index):
        ...
        sketch_a = get_image_by_index(self.paths[name]["sketch"], index)[0]
        sketch_b = get_image_by_index(self.paths[name]["sketch"], next_index)[0]
        ...
        if index == next_index:
            ...
            if random.random() < 0.3:
                ...
                component_method_b = ComponentWrapper.EXTRACT_COLOR
        # extract components
        mask_a, components_a = self.get_component_mask(color_a, sketch_a, path_a, component_method_a)
        mask_b, components_b = self.get_component_mask(color_b, sketch_b, path_b, component_method_b)
        ...
        source_name = "%s_%s" % (source_cm, source_name)
        target_name = "%s_%s" % (target_cm, target_name)

        image_pair_name = "%s_%s_%s" % (key_name, source_name, target_name)
        label_dir = os.path.join(self.root_dir, cut_name, "annotations")
        label_json_path = os.path.join(label_dir, "%s__%s.json" % (target_name, source_name))
        ...
        if index == next_index:
            ...
            if os.path.exists(label_json_path):
                ...
                positive_pairs = positive_pairs[:,[1, 0]] + 1
            else:
        ...
        if len(positive_pairs) > 0:
            if (positive_pairs.max(0)[0] > mask_a.max()-1) or (positive_pairs.max(0)[1] > mask_b.max()-1):
                print(path_a, path_b)
                import pdb; pdb.set_trace()
        ...
        if self.kpts:
            h, w = mask_a.shape[:2]
            centroid_a = [a["centroid"] for a in components_a]
            kpts_a = np.array([np.concatenate([comp['centroid'], [len(comp['coords'])]]) for comp in components_a])
            if len(kpts_a) > 0: 
                kpts_a[:,0] = kpts_a[:,0] / h
                kpts_a[:,1] = kpts_a[:,1] / w
                kpts_a[:,2] = kpts_a[:,2] / h / w

            h, w = mask_b.shape[:2]
            centroid_b = [b["centroid"] for b in components_b]
            kpts_b = np.array([np.concatenate([comp['centroid'], [len(comp['coords'])]]) for comp in components_b])
            if len(kpts_b) > 0: 
                kpts_b[:,0] = kpts_b[:,0] / h
                kpts_b[:,1] = kpts_b[:,1] / w
                kpts_b[:,2] = kpts_b[:,2] / h / w
