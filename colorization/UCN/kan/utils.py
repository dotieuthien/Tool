...
def find_direction_v2(img, rows, cols, D, max_traveled_pixel=20, min_get_both=8, max_d=0.8):
    """
    max_traveled_pixel=5
    :param img:
    :param rows:
    :param cols:
    :param D:
    :param max_traveled_pixel:
    :param min_get_both:
    :param max_d:
    :return:
    """
    def _find(row, col):
        ...
        index = np.argsort(_fs)[::-1][:2]
        _vs, _fs = _vs[index], _fs[index]

        if (len(_vs) >= 2) and (_fs[0] >= _fs[1] > min_get_both) and (_fs[1] > max_d * _fs[0]):
            vs = _vs[0] + _vs[1]
            return vs
        return _vs[0]
...
def is_intersection_point(row, col, cv2_im, max_traveled_pixel=30):
    """
...
def choose_pair_by_distance(rows, cols, max_distance, return_matrix=False):
    ...
    min_ids = np.argsort(distance, axis=-1)
    # np.argmin(distance, axis=-1) # n_samples,
...
def connect_keypoint_to_boundary(point1, cv2_im, max_distance=6, direction_dct=None):
    ...
    next_rs = [r + _ * direction[0] for _ in range(1, n_iter + 1)]
    next_cs = [c + _ * direction[1] for _ in range(1, n_iter + 1)]
    next_px = [cv2_im[_r, _c] if (0 <= _r < h and 0 <= _c < w) else 0 for _r, _c in zip(next_rs, next_cs)]

    if 255 not in next_px:
        return -1, -1
    else:
        _id = next_px.index(255)
...
def can_connect_two_points(point1, point2, cv2_im):
    ...
    for r, c in zip(line_rs, line_cs):
        ...
        if is_in_neighbor(r, c, point1[0], point1[1], 1):
            continue
        if is_in_neighbor(r, c, point2[0], point2[1], 1):
            continue
