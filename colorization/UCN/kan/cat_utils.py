...
def cat_find_bound(ref_colored_img, start, stop, step):
...
def cat_crop_image(img_lst, upper_bound, lower_bound):
    """Crops all the images in the image list.
    Args:
        img_lst (numpy array list): list of image arrays to be cropped
        upper_bound (int): the starting row of the cropped image
...
def cat_preprocess_image(sketch, color_image, diff=200):
    ...
    Args:
        ...
        color_image(numpy array): 3_D reference colored image array
    ...
    upper_bound = cat_find_bound(color_image, 0, diff, 1)
    lower_bound = cat_find_bound(color_image, height - 1, height - diff, -1) + 1
    preprocessed_img = cat_crop_image([sketch, color_image], upper_bound, lower_bound)
