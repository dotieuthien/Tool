...
def load_config(path):
    ...
    Args:
        path (str): path to ini config file
    ...
    # Credentials config (aws and neptune)
    credential_section = "CREDENTIALS"
    dict_config['aws_region'] = get_config_value(config.get, credential_section, "REGION_NAME", None)
    dict_config['aws_access_key'] = get_config_value(config.get, credential_section, "AWS_ACCESS", None)
    dict_config['aws_secret_access_key'] = get_config_value(config.get, credential_section, "AWS_SECRET_ACCESS", None)
    dict_config['neptune_token'] = get_config_value(config.get, credential_section, "neptune_token", None)

    # Parameters to train UCN model
    ...
    # dict_config["neptune_token"] = get_config_value(config.get, common_section, "neptune_token", None)
    ...
    train_section = "TRAIN"
    dict_config["learning_rate"] = get_config_value(config.getfloat, train_section, "learning_rate", 0.001)
    dict_config["batch_size"] = get_config_value(config.getint, train_section, "batch_size", 1)
    dict_config["num_epochs"] = get_config_value(config.getint, train_section, "num_epochs", 41)
    dict_config["step_size"] = get_config_value(config.getint, train_section, "step_size", 10)
    dict_config["print_freq"] = get_config_value(config.getint, train_section, "print_freq", 100)
    dict_config["save_freq"] = get_config_value(config.getint, train_section, "save_freq", 1)
